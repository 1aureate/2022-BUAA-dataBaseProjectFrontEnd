<template>
    <el-container direction="vertical" v-if="this.$store.state.isLogin=='true'">
        ...
        >
            ...
            <el-table-column prop="cname" label="教练" />
            ...
            <el-table-column label="" align="center" width="80">
                ...
                <el-button  type="success" size="mini" :disabled="scope.row.tstatus=='拒绝' || scope.row.tstatus=='通过' || formal == 'false'"  @click="handleRefuseTC(scope.row.tcid)"> {{scope.row.confirm?'已否决':'否决'}}</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        >
            ...
            <el-table-column label="" align="center" width="80">
                <template slot-scope="scope">
                <el-button  type="success" size="mini" :disabled="scope.row.cstatus=='拒绝' || scope.row.cstatus=='生效' || formal == 'false'" @click="handleRefuseCT(scope.row.ctid)"> {{scope.row.confirm?'已否决':'否决'}}</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-footer style="text-align:center;">
...
export default {
    ...
    methods: {
        getTableData1() {
            this.$axios.post('http://127.0.0.1:5000/test/data/ceoTC', 
            {
        ...
        ,
        getTableData2() {
            this.$axios.post('http://127.0.0.1:5000/test/data/ceoCT', 
            {
                uid: this.$store.state.uid,
        ...
        handleRefuseTC(id) {
            this.$axios.post('http://127.0.0.1:5000/test/action/ceoRefuseTC', 
            {
                tcid: id,
                isLogin: this.$store.state.isLogin,
            }).then(
        ...
        handleRefuseCT(id) {
            this.$axios.post('http://127.0.0.1:5000/test/action/ceoRefuseCT', 
            {
                ctid: id,
                isLogin: this.$store.state.isLogin,
            }).then(
